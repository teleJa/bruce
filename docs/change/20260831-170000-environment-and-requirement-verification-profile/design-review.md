...
- Objective: 建立只记录用户提供并确认、用于支撑开发和测试的运行拓扑与操作声明、默认未确认的 Environment Profile，并将 Verification Profile 改造为强制绑定 requirements.md 的需求级验收与修复策略；支持从 confirmed Profile 派生受边界约束的 Environment Operation Manifest。
- Scope: 两类 Profile 的用户声明拓扑 schema、部署/构建/生命周期/依赖/网络/身份/数据/配置边界、账号与 Credential 安全、能力选择、用户确认、stale、阻塞恢复、本地 `.env`、派生 Operation Manifest、模板、文档和契约测试。
- Implementation boundary: 只实现 Bruce supporting skills、Profile lifecycle/security references、templates、workflow/README 文档、本地 `.env` helpers、操作 Manifest validator/template 和契约测试；Environment Profile 不扫描仓库填充事实，不执行项目 Adapter、CNB/部署/客户端操作、业务代码或远程环境操作。
...
- Acceptance and verification coverage: pass — AC-001 至 AC-011 均映射到用户声明 Profile 拓扑 schema、需求输入、环境/账号/能力映射、确认、stale、拓扑操作声明、派生 Operation Manifest、来源 Profile exact binding、暂停恢复或 Completion ownership 场景；对抗性 validator 测试覆盖 repository metadata、原始值和风险降级绕过。
...
- Smallest next action: 按 T-002 revision 4 和 T-005 revision 1 校验开发/测试拓扑与派生 Manifest contract；仅在 Profile exact confirmed 后生成项目级 Manifest。
