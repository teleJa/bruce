...
The selected Agent and every preflight dimension have evidence; `partial` never becomes a claimed
...
| Screenshot/browser evidence is unavailable | Visual automation remains pending and evidence is `unavailable` or `manual-only` | Require explicit user inspection for confirmation and retain the limitation |
...
- Provider: manifest-template and artifact-policy contract tests.
