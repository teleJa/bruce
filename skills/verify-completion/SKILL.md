...
6. Confirm requested delivery actions were authorized and completed, or clearly remain outside the
   completed boundary.
7. When documentation changed, require a current D0 `pass`. Also require a current D1 `通过`, or an
...
9. Select the review mode proportionally:
...
Return review mode, verdict, scenario-level acceptance-to-evidence mapping, C0 result, scope findings,
verification and repair-loop results, unresolved risks, and the smallest next action. Keep the result
in the current task by default; persist a review artifact only when the user explicitly requests one.
