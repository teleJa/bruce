...
class BruceConfigContractTest(unittest.TestCase):
    def test_workspace_and_template_configure_bounded_repair_loop(self) -> None:
        ...
        for config in (workspace, template):
            with self.subTest(config=config):
                ...
                self.assertEqual(10, config["workflow"]["repair_loop"]["max_rounds"])
                self.assertEqual(5, config["workflow"]["repair_loop"]["max_rounds_per_failure"])
    ...
    def test_config_documentation_defines_safe_bounds_and_defaults(self) -> None:
        ...
        for phrase in (
            ...
            "workflow.repair_loop.max_rounds_per_failure",
            "Both repair keys are independently optional",
            "explicit smaller values are preserved",
            "null, booleans, strings, fractions, zero, negative and out-of-range",
            "max_wait_seconds` is 1 through 60",
            "max_no_progress_polls` is 1 through 2",
            "defaults (`10`, `5`, `60`, and `2`",
    ...
    def test_checkpoint_tracks_repair_round_and_completion_state(self) -> None:
        ...
        self.assertIn("max_rounds: 10", checkpoint)
        self.assertIn("Copy resolved workflow.repair_loop.max_rounds", checkpoint)
        self.assertIn("workflow.repair_loop.max_rounds_per_failure", checkpoint)
