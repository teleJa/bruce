...
- Scope：Environment Profile 的开发/测试运行拓扑（部署、构建、生命周期、依赖、网络、身份、数据、配置/凭证、preflight）、确认/失效状态、账号安全边界、需求 Acceptance 映射、派生 Executable Environment Operation Skill、阻塞恢复、静态 validator、模板、文档和契约测试。
...
| T-005 | 生成 Executable Environment Operation Skill | T-002 | AC-010, AC-011 | generated Skill/runner generation, execution, stale-binding and boundary tests |
...
4. confirmed Environment Profile 可在用户显式请求时派生项目级 Executable Environment Operation Skill；生成的 Skill 只封装已确认操作，不扩大授权。
