...
必须返回待执行的强制 `design-gate` handoff，由 Bruce/调用方按以下批次规则合并执行，无需用户追加指令：

- 批次只包含当前已授权设计范围内、按独立触发条件确定的必需工件；不能因收到 handoff 而追加工件或扩大范围。
- 同一批次需要多个工件时，先完成这些工件及各自本地文档检查，再合并执行一次 Gate；不在每个 writer 返回后重复检查尚待生成的其余工件。
- 单工件批次在本地检查完成后就绪；多工件批次在最后一个必需工件及其本地检查完成后就绪。批次就绪时，必须在同一轮内立即执行 Gate，无需用户追加指令。
- 必需工件缺失、生成失败或本地检查仍有问题时，不得进入依赖它的实现；可在已授权范围内继续生成或修复，真实阻塞则报告缺口，不能为了合并而无限等待或跳过必需工件。待执行 handoff 不是 Design verdict。
- Gate 已执行后，必要修复或治理型设计发生变化时，仍按现有规则重新评审；“一次”不表示复用失效的 Gate 结果。

该自动衔接不把 Design verdict 转移给工件 Skill，也不扩大原任务的实现授权；design-only 仍在设计工件和 Gate 结果后停止。
