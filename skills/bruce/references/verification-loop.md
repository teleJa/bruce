...
# Compare l1_repair_rounds with configured workflow.repair_loop.max_rounds_per_failure (default 5).
repair_loop:
  max_rounds: 10 # Copy resolved workflow.repair_loop.max_rounds; 10 is the default, not an override.
  current_round: 0
...
dependent tasks continue. Batch repairs read `workflow.repair_loop.max_rounds_per_failure` from the
applicable `.bruce/config.yaml` (default 5), following [failure-recovery.md](failure-recovery.md); they never consume the Completion-only
